...
--Stored procedure 
CREATE OR REPLACE PROCEDURE PRINT_BANK_ACCTS( A_ID)
IS 
BEGIN
...
CREATE OR REPLACE PROCEDURE CALC_NET_INCOME (B_ID IN BANK_ACCT.BANK_ACCT_ID%TYPE)
RETURN NUMBER
IS
SUM_W NUMBER;
SUM_D NUMBER;
NET NUMBER;
BEGIN
    SELECT SUM(MODIFIER) INTO SUM_W FROM TRANSACTIONS WHERE TRANSACTION_TYPE = 'withdrawal';
    SELECT SUM(MODIFIER) INTO SUM_D FROM TRANSACTIONS WHERE TRANSACTION_TYPE = 'deposit';
    NET := SUM_D + SUM_W;
    RETURN NET;
END;
